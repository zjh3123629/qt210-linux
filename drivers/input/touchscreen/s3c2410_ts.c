			...
			const char *buf, size_t count)
{
	...
	printk( "CRZ %s : cal_x_max : %d cal_y_max : %d "
			"cal_param : %d %d %d %d %d %d %d\n", __func__,
			b->cal_x_max, b->cal_y_max, b->cal_param[0],
			b->cal_param[1], b->cal_param[2], b->cal_param[3],
			b->cal_param[4], b->cal_param[5], b->cal_param[6]);	
...
static void touch_timer_fire(unsigned long data)
{
	...
	unsigned long data1;
	bool down;
	unsigned int temp_x,temp_y;//CRZ 
	...
	//		printk( "CRZ touch before -%s: X=%lu, Y=%lu, count=%d\n",
	//			__func__, ts.xp, ts.yp, ts.count);

#if defined(CONFIG_FB_S3C_LB070WV6)
#if defined(CONFIG_FB_QT_90_ROTATION)
	          temp_y = ts.xp;
                        temp_x =  ts.yp;	
          //              printk( "CRZ touch swap after -%s: X=%lu, Y=%lu, count=%d\n",
            //                    __func__, temp_x, temp_y, ts.count);
	
                        ts.yp = temp_y;
			
                        ts.xp = 480-temp_x;

              //          printk( "CRZ touch after 2  -%s: X=%lu, Y=%lu, count=%d\n",
                //                __func__, ts.xp, ts.yp, ts.count);
...
static int __devinit s3c2410ts_probe(struct platform_device *pdev)
{
	...
	if (info->cal_x_max == 0 || info->cal_y_max == 0) {
		...
		ts.cal_enable = 1;
#if defined(CONFIG_FB_QT_90_ROTATION)//CRZ 90 Rotation		
		input_set_abs_params(ts.input, ABS_X, 0, info->cal_y_max, 32, 0);
		input_set_abs_params(ts.input, ABS_Y, 0, info->cal_x_max, 32, 0);
#else
		input_set_abs_params(ts.input, ABS_X, 0, info->cal_x_max, 32, 0);
		input_set_abs_params(ts.input, ABS_Y, 0, info->cal_y_max, 32, 0);
#endif		
	}
             printk("CRZ %s info->cal_x_max=%ld, info->cal_y_max=%ld\n",__func__, info->cal_x_max,info->cal_y_max);
