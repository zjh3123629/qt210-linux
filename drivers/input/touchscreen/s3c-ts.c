...
static char *s3c_ts_name = "s5pc210_ts";
...
static void touch_timer_fire(unsigned long data)
{
	...
	if (updown) {
		if (ts->count) {
			...
			input_report_abs(ts->dev, ABS_PRESSURE, 255);
			input_sync(ts->dev);
#else
...
static int __init s3c_ts_probe(struct platform_device *pdev)
{
	...
	if (s3c_ts_cfg->resol_bit==12) {
#if 1//crztech
        input_set_abs_params(ts->dev, ABS_X, 100,3980, 32, 0);
        input_set_abs_params(ts->dev, ABS_Y, 200,3760, 32, 0);
#else		
